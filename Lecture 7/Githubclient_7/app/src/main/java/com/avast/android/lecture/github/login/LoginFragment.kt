...
import com.avast.android.lecture.github.BaseApp
...
class LoginFragment: Fragment() {
    ...
    private fun fireNotification(login: String) {
        ...
        activity?.let {
            ...
            val notification = NotificationCompat.Builder(it, BaseApp.NOTIFICATION_CHANNEL_MAIN_ID)
            ...
            NotificationManagerCompat.from(it).notify(BaseApp.NOTIFICATION_HELLO_WORLD_ID, notification)
        }
    }

    /**
     * Validate login text before API call.
     */
    private fun isLoginValid(loginTest: String): Boolean =
        when {
            loginTest.isEmpty() -> false
            loginTest == "test:invalid-user" -> false
            else -> true
        }
}