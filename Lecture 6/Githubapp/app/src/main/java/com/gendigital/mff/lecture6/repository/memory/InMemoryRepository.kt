package com.gendigital.mff.lecture6.repository.memory

import com.gendigital.mff.lecture6.data.GithubRepository
import com.gendigital.mff.lecture6.data.User
import com.gendigital.mff.lecture6.repository.Repository
...
class InMemoryRepository: Repository {
    ...
    override suspend fun getUser(username: String): Result<User> = runBlocking {
        ...
        if (user == null) Result.failure(IOException("User not found")) else Result.success(user)
    }

    override suspend fun getUserRepository(username: String): Result<List<GithubRepository>> = runBlocking {
        ...
        user.onSuccess {
            ...
            if (repositories == null) Result.failure(IOException("Failed to get repositories")) else Result.success(repositories)

        }.onFailure {
            Result.failure<Throwable>(it)
        }

        Result.failure(IOException("Failed to get repositories"))
    }
}